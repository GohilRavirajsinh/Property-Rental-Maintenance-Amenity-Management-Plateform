...
const ManageAmenities = () => {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                ...
                const ownerProps = response.data.filter(p => p.owner?._id === user._id || p.owner === user._id);
    ...
    useEffect(() => {
        const fetchAmenities = async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const response = await axios.get(${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/amenities/property/, {
                    headers: { Authorization: Bearer  }
                });
