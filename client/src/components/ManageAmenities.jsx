...
const ManageAmenities = () => {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                ...
                const ownerProps = response.data.filter(p => p.owner?._id === user._id);
    ...
    useEffect(() => {
        const fetchAmenities = async () => {
            ...
            try {
                const token = localStorage.getItem('token');
                const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/amenities/property/${selectedProperty}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/amenities/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    return (
        <div className="mt-8 border-t border-gray-200 pt-8">
            ...
            {properties.length === 0 ? (
                <p className="text-gray-500">You don't have any properties yet to manage amenities.</p>
            ) : (
